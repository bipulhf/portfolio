    ...
    <section className={cx(pageContainerClass, 'relative pt-8 sm:pt-10 md:pt-16 lg:pt-20')}>
      <div className="page-enter enter-soft motion-delay-2 relative overflow-hidden rounded-[1.6rem] border-[2.5px] border-ink bg-[linear-gradient(180deg,rgba(255,255,255,0.72),rgba(244,250,249,0.95)),var(--color-paper)] px-5 py-8 shadow-crayon-lg sm:rounded-[2rem] sm:px-6 sm:py-10 md:px-10 md:py-12 lg:px-14 lg:py-16">
        ...
        <div className="relative max-w-3xl">
          <div className="page-enter enter-soft motion-delay-3 mb-4 flex flex-wrap items-center gap-3 font-hand text-base text-ink-soft sm:text-lg md:text-xl">
            <span className="h-0.5 w-8 rounded-full bg-ink-soft sm:w-10" />
            <span>{eyebrow}</span>
          </div>
          <h1 className="section-scribble page-enter enter-soft motion-delay-4 relative inline-block font-display text-[clamp(2.8rem,8vw,5.2rem)] font-bold leading-[0.95] text-ink">
            {title}
          ...
          <p className="page-enter enter-soft motion-delay-5 mt-5 max-w-2xl text-base text-ink-soft md:text-lg">{description}</p>

...
export function MinimalHero() {
  ...
  return (
    <section className="theme-only-minimal py-14 md:py-24" id="top">
      <div className={pageContainerClass}>
        <div className="coding-border overflow-hidden rounded-xl">
          <div className={`flex items-center justify-between gap-4 border-b border-[#3a342e]/8 px-6 py-4 text-[11px] [font-family:var(--minimal-mono)] ${charcoalFaint}`}>
          ...
          <div className="grid gap-10 px-6 py-7 md:grid-cols-[minmax(0,1.4fr)_minmax(14rem,0.7fr)] md:px-8 md:py-9">
            <div className="space-y-5">
              <p className={`text-[11px] uppercase tracking-[0.18em] [font-family:var(--minimal-mono)] ${charcoalFaint}`}>
                {hero.intro}
              </p>

              <div className={`space-y-1 text-[1rem] leading-7 md:text-[1.12rem] [font-family:var(--minimal-mono)] ${charcoalSoft}`}>
                <div className={charcoalStrong}>const engineer = {'{'}</div>
                <div className="pl-4">
                  <span className={charcoalFaint}>name:</span>{' '}
                  <span className={`font-semibold ${charcoalStrong}`}>"{hero.title}"</span>,
                </div>
                <div className="pl-4">
              ...
              <p className={`max-w-2xl text-sm leading-relaxed md:text-base ${charcoalSoft}`}>
                {hero.lead}
              </p>
            ...
            <dl className="grid gap-3 self-start md:pt-1">
              {heroSignals.map(([label, value]) => (
                <div className="rounded-[0.9rem] border border-[#3a342e]/10 bg-white/36 px-4 py-3" key={label}>
                  <dt className={`text-[10px] uppercase tracking-[0.18em] [font-family:var(--minimal-mono)] ${charcoalFaint}`}>
                    {label}
                  </dt>
                  <dd className={`mt-2 text-sm font-medium [font-family:var(--minimal-mono)] ${charcoalStrong}`}>
                    {value}
                  </dd>
                </div>
              ))}

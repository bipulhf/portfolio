  ...
  return (
    <SectionShell accent="skills" id="skills">
      ...
      <div className="mt-12 grid gap-5 lg:grid-cols-[minmax(19rem,0.9fr)_minmax(0,1.1fr)] lg:gap-6">
        ...
        <div className="grid items-stretch gap-[1.125rem] sm:grid-cols-2 lg:gap-[1.25rem]" data-reveal-sequence>
          {SKILL_CATEGORIES.map((category, index) => (
            <div
              className={`${skillTones[index % skillTones.length] ?? skillTones[0]} ${surfaceCardClass} reveal-pop flex min-h-[11.25rem] flex-col justify-between gap-4 rounded-[1.25rem] border-2 border-ink p-5 shadow-[4px_4px_0_var(--color-ink)] hover:-translate-y-[0.3rem] hover:shadow-[7px_9px_0_var(--color-ink)] sm:p-[1.375rem] ${index === SKILL_CATEGORIES.length - 1 ? 'sm:col-span-2 sm:mx-auto sm:w-full sm:max-w-[18.5rem]' : ''}`}
              data-reveal-item
              key={category.title}
            >

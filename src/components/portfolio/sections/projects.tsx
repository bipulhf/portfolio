...
export function Projects({ items }: Readonly<{ items: SerializedProject[] }>) {
  return (
    <SectionShell accent="projects" id="projects">
      ...
      />
      <div className="mt-12 grid gap-7 md:grid-cols-2 lg:grid-cols-6 lg:gap-8" data-reveal-sequence>
        {items.length ? items.map((project, index) => (
          ...
          >
            ...
            <div className={`flex h-full flex-col px-[1.375rem] pb-[1.375rem] pt-5 ${index === 0 ? 'lg:px-7 lg:pb-7 lg:pt-6' : ''}`}>
              <h3 className={index === 0 ? 'type-display-section mb-3 max-w-[12ch] leading-[0.96]' : 'type-display-card-lg mb-2'}>
                {project.title}
              </h3>
              <p className={`type-copy mb-3.5 flex-1 text-[0.98rem] ${index === 0 ? 'max-w-[34ch] text-[1rem]' : ''}`}>{project.summary}</p>

...
export function Blog({ items }: Readonly<{ items: SerializedBlog[] }>) {
  return (
    <SectionShell accent="blog" id="blog">
      ...
      />
      <div
        className="mt-12 grid gap-7 md:grid-cols-2 lg:grid-cols-6"
        ...
        {items.length ? (
          items.map((post, index) => (
            ...
            >
              ...
              <div
                className={`flex h-full flex-col px-[1.375rem] pb-[1.375rem] pt-[1.125rem] ${index === 0 ? "lg:px-7 lg:pb-7 lg:pt-6" : ""}`}
              >
                ...
                <h3
                  className={
                    index === 0
                      ? "type-display-section mb-3 max-w-[13ch] leading-[0.96]"
                      : "type-display-card-md mb-2"
                  }
                >
                  {post.title}
                </h3>
                <p
                  className={`type-copy flex-1 text-[0.98rem] ${index === 0 ? "max-w-[34ch] text-[1rem]" : ""}`}
                >
                  {post.excerpt}
                </p>
